...
function fmtDateTime(ts) {
  // Backward-compatible alias used by the private assistant overlay.
  return fmtTs(ts);
}
...
function logActivityToEntity(entity_type, entity_id, kind, message, meta, opts) {
  ...
  pushActivity(bucket, {
    ...
    agent: (opts && opts.agent) ? String(opts.agent) : "specialist_ps",
    kind: kind || "activity",
    entity_type,
    entity_id,
    message: message || "",
...
function setDecision(type, id, decision, note, opts) {
  ...
  const actor = (opts && opts.actor) ? String(opts.actor) : "Human";
  const agent = (opts && opts.agent) ? String(opts.agent) : "human";
  const ts = (opts && opts.ts) ? String(opts.ts) : nowIso();
...
/* ===== Private assistant overlay store (localStorage) ===== */
...
async function loadAssistantConfig() {
  try {
    if (state.assistant && state.assistant.config) return state.assistant.config;
    const res = await fetch(ASSIST_CONFIG_URL, { cache: "no-store" });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const cfg = await res.json();
    state.assistant ||= {};
    state.assistant.config = cfg;
    return cfg;
  } catch {
    // Non-blocking: assistant still works with built-in defaults
    return null;
  }
}
...
function pushAssistChat(ev) {
  ...
  bucket.chat.push({
    ...
    actor: ev.actor || (ev.role === "assistant" ? "RAPSOBOT" : "Human"),
    agent: ev.agent || (ev.role === "assistant" ? "specialist_ps" : "human"),
    message: ev.message || "",
    meta: ev.meta || {},
  });
...
function hasPendingAssistantActions() {
  const p = getPendingAssistantActions();
  return p && Array.isArray(p.tool_calls) && p.tool_calls.length;
}
...
function buildUiSnapshot({ scope = "unknown", type = null, id = null } = {}) {
  // lightweight + stable: do not depend on DOM structure
  return {
    scope,
    now: nowIso(),
    display_depth: state.displayDepth || "situations",
    filters: {
      verdict: state.verdictFilter || "ALL",
      search: state.search || "",
    },
    selection: {
...
/* ===== LLM response parsing helpers (robust to fenced JSON blocks) ===== */
...
/* ===== Unwrap OpenAI Responses API payloads (n8n may return full 'response' objects) ===== */
function _unwrapOpenAIResponsesText(out) {
  try {
    if (!out || typeof out !== "object") return null;
    // OpenAI Responses API shape: { object:"response", output:[{type:"message", content:[{type:"output_text", text:"..."}]}] }
    ...
    const t = (ot && typeof ot.text === "string") ? ot.text : null;
...
function _normalizeLlmRawToObject(rawText) {
  ...
  // Best-effort: try parsing the largest {...} slice
...
function _pickReplyMarkdown(out, rawText) {
  // 1) Direct contract object
  if (out && typeof out === "object") {
    const r = (out.reply_markdown ?? out.reply ?? out.message ?? out.content ?? "");
    if (typeof r === "string" && r.trim()) return r.trim();

    // 2) OpenAI Responses API wrapper (n8n might return the full response object)
  ...
  // 3) Raw webhook text: try to parse JSON or fenced JSON
...
function _pickToolCalls(out, rawText) {
  if (out && typeof out === "object") {
    const tc = Array.isArray(out.planned_actions) ? out.planned_actions
      : (Array.isArray(out.tool_calls) ? out.tool_calls
      : (Array.isArray(out.tool_call) ? out.tool_call : null));
    if (tc) return tc;

    const wrapped = _unwrapOpenAIResponsesText(out);
    if (wrapped) {
      const obj = _normalizeLlmRawToObject(wrapped);
      if (obj) return _pickToolCalls(obj, null);
    }
  }
...
async function askRapsoAndAppendReply({ type, id, humanMessage }) {
  ...
  addAgentComment(type, id, "RAPSOBOT est en train de réfléchir…", "specialist_ps", { pending: true, request_id });
  refreshAll();
  ...
  try {
    ...
    // Robust parse: webhook may return JSON, or a fenced ```json ...``` block.
    let out = null;
    try { out = JSON.parse(txt); } catch { out = null; }

    const reply = _pickReplyMarkdown(out, txt);
    if (!reply) {
  ...
  const bodyHtml = pending
    ? `<div>
        <div class="rapso-wait">
          ...
          <span class="rapso-shimmer">${escapeHtml(cleanMd || "RAPSOBOT réfléchit…")}</span>
        </div>
        <div class="rapso-wait-sub">On revient vers vous dans quelques instants.</div>
       </div>`
    : mdToHtml(cleanMd);

  const avatar = (role === "user")
    ? `<div class="gh-comment-avatar gh-comment-avatar--help gh-comment-avatar--human" aria-hidden="true">${SVG_AVATAR_HUMAN}</div>`
    : `<div class="gh-comment-avatar gh-comment-avatar--help gh-comment-avatar--rapso mono" aria-hidden="true">R</div>`;
  return `
    <div class="thread-item thread-item--comment thread-item--comment--flush" data-thread-kind="help" data-thread-idx="-1">
      <div class="thread-wrapper">
        <div class="gh-comment gh-comment--help gh-comment--${role}">
          ${avatar}
          <div class="gh-comment-box gh-comment-box--help">
            <div class="gh-comment-header gh-comment-header--help">
              <div class="gh-comment-header__left">
                <span class="gh-comment-header__who">${escapeHtml(who)}</span>
                <span class="gh-comment-header__sep">•</span>
                <span class="gh-comment-header__ts">${escapeHtml(ts)}</span>
              </div>
              <div class="gh-comment-header__right">
                ${pending ? `<span class="gh-comment-header__pending">…</span>` : ``}
              </div>
            </div>
            <div class="gh-comment-body gh-comment-body--help">${bodyHtml}</div>
          </div>
        </div>
      </div>
    </div>
...
function showEphemeralHelpThread(rootEl, { userMd, assistantPendingMd = "RAPSOBOT réfléchit…", ttlMs = 60_000 } = {}) {
  if (!rootEl) return null;

  const anchor =
    rootEl.querySelector(".gh-thread") ||
    rootEl.querySelector(".gh-thread-wrap") ||
    rootEl;

  const wrap = document.createElement("div");
  wrap.className = "help-ephemeral";
  wrap.innerHTML = `
    ${_helpFurtiveCommentHtml({ role: "user", bodyMd: userMd || "" })}
    <div class="help-ephemeral__reply">
      ${_helpFurtiveCommentHtml({ role: "assistant", bodyMd: assistantPendingMd || "", pending: true })}
    </div>
  `;
...
async function askHelpEphemeral({ rootEl, type, id, humanMessage, scope = "details" } = {}) {
  ...
  const ui_snapshot = buildUiSnapshot({ scope, type, id });

  const ctx = buildRapsoContextBundle(type, id, q);
  if (!ctx) return;

  ctx.help_mode = true;
  ctx.ui_snapshot = ui_snapshot;
  ctx.user_message = [
  ...
  const request_id = `help_${Date.now()}_${type}_${id}`;
  const payload = { agent: "specialist_ps", request_id, context: ctx };
...
/* ===== Private assistant overlay: context + tool-call execution ===== */
function buildRapsoAssistantContext(userMessage) {
  ...
  const pickDSAvis = (avisIds) => {
    const out = [];
    for (const aidAny of (avisIds || [])) {
      const a = avById.get(idFromAny(aidAny));
      if (!a) continue;
      const v = String(getEffectiveAvisVerdict(a.avis_id) || a.verdict || "").toUpperCase();
      if (v !== "D" && v !== "S") continue;
      out.push({
        ...
        severity: a.severity || "",
        topic: a.topic || "",
        summary: summarizeOneLine(a.message || "", 160),
      });
    }
    return out;
  };

  const situations = (d.situations || []).map((s) => {
    const problems = (s.problem_ids || [])
      .map((pidAny) => pbById.get(idFromAny(pidAny)))
      .filter(Boolean)
      .map((pb) => {
        const dsAvis = pickDSAvis(pb.avis_ids || []);
        return {
          sujet_id: pb.problem_id,
          status: String(getEffectiveProblemStatus(pb.problem_id) || pb.status || "open").toUpperCase(),
          topic: pb.topic || "Non classé",
    ...
    return {
      situation_id: s.situation_id,
      status: String(getEffectiveSituationStatus(s.situation_id) || s.status || "open").toUpperCase(),
      title: s.title || "(sans titre)",
      priority: s.priority || "",
      sujets: problems,
    };
  ...
  return {
    ...
    execution_policy: {
      ...
      authorization_phrase: ASSIST_AUTH_PHRASE,
      behavior: [
        "Poser des questions si la demande est ambiguë ou incomplète avant de proposer des actions.",
        "Toujours résumer la demande et lister les actions exactes prévues avant toute exécution.",
        "Ne jamais exécuter d’actions sans autorisation explicite (phrase ou bouton).",
      ],
    },
    tool_call_contract: {
      mode: "A",
      expected: {
        ...
        tool_calls: "optional array of tool calls to execute",
      },
      allowed_tools: [
  {
    name: "bulk_set_avis_verdict_by_sujet",
    args: { sujet_id: "string", verdict: "F|D|S|HM|PM|SO", note: "string", add_comment_to_sujet: "boolean" }
  },
  {
    name: "set_avis_verdict",
    args: { avis_id: "string", verdict: "F|D|S|HM|PM|SO", note: "string", add_comment_to_sujet: "boolean" }
  },

  // Sujets (unitaire / bulk)
  {
    name: "set_sujet_status",
    args: { sujet_id: "string", status: "open|closed", note: "string", add_comment_to_situation: "boolean" }
  },
  {
    name: "bulk_set_sujet_status_by_situation",
    args: { situation_id: "string", status: "open|closed", note: "string", add_comment_to_situation: "boolean" }
  },
  {
    name: "bulk_set_sujet_status",
    args: { sujet_ids: "array<string>", status: "open|closed", note: "string", add_comment_to_situation: "boolean" }
  },

  // Situations (unitaire / bulk)
  {
    name: "set_situation_status",
    args: { situation_id: "string", status: "open|closed", note: "string" }
  },
  {
    name: "bulk_set_situation_status",
    args: { situation_ids: "array<string>", status: "open|closed", note: "string" }
  }
]
    },
    user_message: String(userMessage || "").trim(),
...
function findAvisIdsForProblem(pid) {
  const pb = findProblem(pid);
  if (!pb) return [];
  return (pb.avis_ids || []).map(idFromAny).filter(Boolean);
}

function applyAssistantToolCall(tc, opts) {
  const name = String(tc?.name || tc?.tool || "").trim();
  const args = tc?.arguments || tc?.args || {};
  const actor = (opts && opts.actor) ? String(opts.actor) : "RAPSOBOT";
  const agent = (opts && opts.agent) ? String(opts.agent) : "specialist_ps";
  const ts = nowIso();

  if (!name) return { ok: false, error: "missing tool name" };

  if (name === "set_avis_verdict") {
    const avis_id = String(args?.avis_id || "").trim();
    const verdict = String(args?.verdict || "").toUpperCase();
    const note = String(args?.note || "").trim();
    if (!avis_id || !verdict) return { ok: false, error: "missing avis_id or verdict" };

    // Apply decision as-if Human, but attributed to RAPSOBOT (per requirement: historized even when triggered by Rapso)
    setDecision("avis", avis_id, `VALIDATED_${verdict}`, note, { actor, agent, ts });

    // Log activity to all concerned timelines (avis + parent sujet + parent situation)
    const d = state.data;
    if (d) {
      const parents = buildParents(d);
      const pid = parents.avisToProblem.get(avis_id) || null;
      const sid = parents.avisToSituation.get(avis_id) || null;

      logActivityToEntity("avis", avis_id, "avis_verdict_changed", note, { avis_id, to: verdict }, { actor, agent, ts });
      if (pid) logActivityToEntity("problem", pid, "avis_verdict_changed", note, { avis_id, to: verdict }, { actor, agent, ts });
      if (sid) logActivityToEntity("situation", sid, "avis_verdict_changed", note, { avis_id, to: verdict, problem_id: pid }, { actor, agent, ts });

      if (args?.add_comment_to_sujet && pid) {
        addAgentComment("problem", pid, note ? `Validé (par Human) — ${note}` : "Validé (par Human).", agent, { tool: name, ts, from_assistant: true });
      }
    }
  ...
  if (name === "bulk_set_avis_verdict_by_sujet") {
    const sujet_id = String(args?.sujet_id || "").trim();
    const verdict = String(args?.verdict || "").toUpperCase();
    const note = String(args?.note || "").trim();
    if (!sujet_id || !verdict) return { ok: false, error: "missing sujet_id or verdict" };
    ...
    if (args?.add_comment_to_sujet) {
      addAgentComment("problem", sujet_id, note ? `Validé (par Human) — ${note}` : "Validé (par Human).", agent, { tool: name, ts, from_assistant: true });
      // Also log to situation timeline for traceability
      const d = state.data;
      if (d) {
        const sid = buildParents(d).problemToSituation.get(sujet_id) || null;
        if (sid) logActivityToEntity("situation", sid, "bulk_avis_validated", note, { problem_id: sujet_id, to: verdict }, { actor, agent, ts });
      }
    }

    return { ok: true, meta: { updated: avisIds.length } };
  }


if (name === "set_sujet_status") {
  const sujet_id = String(args?.sujet_id || "").trim();
  const statusRaw = String(args?.status || "").trim().toLowerCase();
  const status = (statusRaw === "closed") ? "closed" : (statusRaw === "open" ? "open" : "");
  const note = String(args?.note || "").trim();
  if (!sujet_id || !status) return { ok: false, error: "missing sujet_id or status" };

  const decision = status === "closed" ? "CLOSED" : "REOPENED";
  setDecision("problem", sujet_id, decision, note, { actor, agent, ts });

  const d = state.data;
  if (d) {
    const parents = buildParents(d);
    const sid = parents.problemToSituation.get(sujet_id) || null;

    logActivityToEntity("problem", sujet_id, "issue_status_changed", note, { to: status }, { actor, agent, ts });
    if (sid) logActivityToEntity("situation", sid, "issue_status_changed", note, { problem_id: sujet_id, to: status }, { actor, agent, ts });

    if (args?.add_comment_to_situation && sid) {
      addAgentComment("situation", sid, note ? `Changement de statut sujet (${sujet_id}) — ${note}` : `Changement de statut sujet (${sujet_id}).`, agent, { tool: name, ts, from_assistant: true });
    }
  }
...
if (name === "bulk_set_sujet_status_by_situation") {
  const situation_id = String(args?.situation_id || "").trim();
  const statusRaw = String(args?.status || "").trim().toLowerCase();
  const status = (statusRaw === "closed") ? "closed" : (statusRaw === "open" ? "open" : "");
  const note = String(args?.note || "").trim();
  if (!situation_id || !status) return { ok: false, error: "missing situation_id or status" };

  const sit = findSituation(situation_id);
  const ids = (sit?.problem_ids || []).map(idFromAny).filter(Boolean);

  for (const pid of ids) {
    applyAssistantToolCall({ name: "set_sujet_status", arguments: { sujet_id: pid, status, note, add_comment_to_situation: false } }, opts);
  }

  if (args?.add_comment_to_situation) {
    addAgentComment("situation", situation_id, note ? `Changement de statut en masse des sujets — ${note}` : "Changement de statut en masse des sujets.", agent, { tool: name, ts, from_assistant: true });
  }

  return { ok: true, meta: { updated: ids.length } };
}

if (name === "bulk_set_sujet_status") {
  const sujet_ids = Array.isArray(args?.sujet_ids) ? args.sujet_ids : [];
  const statusRaw = String(args?.status || "").trim().toLowerCase();
  const status = (statusRaw === "closed") ? "closed" : (statusRaw === "open" ? "open" : "");
  const note = String(args?.note || "").trim();
  if (!sujet_ids.length || !status) return { ok: false, error: "missing sujet_ids or status" };

  const ids = sujet_ids.map(idFromAny).filter(Boolean);
  for (const pid of ids) {
    applyAssistantToolCall({ name: "set_sujet_status", arguments: { sujet_id: pid, status, note, add_comment_to_situation: false } }, opts);
  }

  // Optional: if all sujets share same parent situation, add comment once there
  if (args?.add_comment_to_situation && ids.length) {
    const d = state.data;
    const parents = d ? buildParents(d) : null;
    const sid = parents ? parents.problemToSituation.get(ids[0]) : null;
    let same = true;
    if (parents && sid) {
      for (const pid of ids) {
        if ((parents.problemToSituation.get(pid) || null) !== sid) { same = false; break; }
      }
    } else {
      same = false;
    }
    if (same && sid) {
      addAgentComment("situation", sid, note ? `Changement de statut en masse des sujets (${ids.length}) — ${note}` : `Changement de statut en masse des sujets (${ids.length}).`, agent, { tool: name, ts, from_assistant: true });
    }
  }
...
if (name === "set_situation_status") {
  ...
  const decision = status === "closed" ? "CLOSED" : "REOPENED";
  setDecision("situation", situation_id, decision, note, { actor, agent, ts });
  logActivityToEntity("situation", situation_id, "issue_status_changed", note, { to: status }, { actor, agent, ts });

  return { ok: true };
}

if (name === "bulk_set_situation_status") {
  const situation_ids = Array.isArray(args?.situation_ids) ? args.situation_ids : [];
  const statusRaw = String(args?.status || "").trim().toLowerCase();
  const status = (statusRaw === "closed") ? "closed" : (statusRaw === "open" ? "open" : "");
  const note = String(args?.note || "").trim();
  if (!situation_ids.length || !status) return { ok: false, error: "missing situation_ids or status" };

  const ids = situation_ids.map(idFromAny).filter(Boolean);
  for (const sid of ids) {
    applyAssistantToolCall({ name: "set_situation_status", arguments: { situation_id: sid, status, note } }, opts);
  }
  return { ok: true, meta: { updated: ids.length } };
}
...
async function askRapsoAssistant({ userMessage, meta = {} }) {
  const ctx = buildRapsoAssistantContext(userMessage);
  const request_id = `assist_${Date.now()}`;

  // placeholder in assistant chat
  pushAssistChat({ role: "assistant", actor: "RAPSOBOT", agent: "specialist_ps", message: "RAPSOBOT analyse votre demande…", meta: { ...(meta || {}), pending: true, request_id } });
  renderAssistantOverlay();

  const payload = {
    agent: "specialist_ps",
    request_id,
    channel: "private_assistant",
    tool_call: {
      name: "rapso_assistant",
      arguments: ctx,
    },
  };
  ...
  try {
    ...
    for (let i = list.length - 1; i >= 0; i--) {
      const ev = list[i];
      if (String(ev?.meta?.request_id || "") === String(request_id)) {
        ev.message = reply;
        ev.meta = { ...(ev.meta || {}), pending: false };
        break;
      }
    }
    saveAssistStore(store);

    // Store planned actions (do NOT execute automatically)
    const toolCalls = _pickToolCalls(out, txt);
    ...
    if (Array.isArray(toolCalls) && toolCalls.length) {
      ...
      // If the model forgot to ask for authorization, enforce it with a system hint
      pushAssistChat({
        role: "system",
        actor: "Système",
        agent: "system",
        message: `⚠️ Des actions sont prêtes à être exécutées. Pour déclencher l’exécution, cliquez sur **Autorisation directeur** ou tapez **${ASSIST_AUTH_PHRASE}**.`,
      });
    } else {
    ...
    for (let i = list.length - 1; i >= 0; i--) {
      const ev = list[i];
      if (String(ev?.meta?.request_id || "") === String(request_id)) {
        ev.message = `_(error: ${errMsg})_`;
        ev.meta = { ...(ev.meta || {}), pending: false, error: true };
        break;
      }
    }
    saveAssistStore(store);
    renderAssistantOverlay();
    showError(`Assistant privé: échec de l'appel LLM (${errMsg})`);
  }
}
...
function miniAuthorIconHtml(agent) {
  ...
  // Agent/system activities are shown as RAPSOBOT in the timeline
...
if (t === "ACTIVITY") {
      ...
      // Local display name (do NOT rely on displayName from the COMMENT branch)
...
const note = String(e?.message || "").trim();
        ...
        <div class="comment-general-block">
          ...
          <div class="comment-box gh-comment-boxwrap">
            ...
            <div class="comment-editor" id="${id("commentEditor")}">
              <textarea id="${id("humanComment")}" class="textarea" placeholder="Réponse humaine (Markdown) — mentionne @rapso pour demander l’avis de l’agent. Ex: « @rapso peux-tu vérifier ce point ? »" data-placeholder-default="Réponse humaine (Markdown) — mentionne @rapso pour demander l’avis de l’agent. Ex: « @rapso peux-tu vérifier ce point ? »"></textarea>
            </div>
  ...
  const wireHost = (root) => {
    ...
    if (ta) {
      ...
      // Ctrl+Enter to send (keeps Enter for new lines)
      ta.addEventListener("keydown", (ev) => {
        if (ev.key === "Enter" && (ev.ctrlKey || ev.metaKey)) {
          ev.preventDefault();
          const btn = root.querySelector("[data-action='add-comment']:not([disabled])") || root.querySelector("[data-action='add-comment']");
          if (btn) btn.click();
        }
      });
      syncCommentBtnState();
...
root.querySelectorAll("[data-action='toggle-help']").forEach((btn) => {
  btn.onclick = () => {
    if (!ta) return;
    const on = String(ta.dataset.helpMode || "0") === "1" ? false : true;
    ta.dataset.helpMode = on ? "1" : "0";
    btn.setAttribute("aria-pressed", on ? "true" : "false");
    btn.classList.toggle("is-on", on);

    // Visual "help" mode on the local comment box (details / modal / drilldown)
    const box = ta.closest ? ta.closest(".comment-box") : null;
    if (box) box.classList.toggle("gh-comment-box--help", on);
    const tabs = box ? box.querySelector(".comment-tabs") : (ta.closest ? ta.closest(".comment-general-block")?.querySelector?.(".comment-tabs") : null);
    if (tabs) tabs.classList.toggle("gh-comment-header--help", on);

    // Only local UI hinting (no persistence)
    ta.setAttribute("placeholder", on
      ? "Help (éphémère) — décrivez l’écran / l’action souhaitée. /help ou @help fonctionne aussi."
      : ta.getAttribute("data-placeholder-default") || ta.getAttribute("placeholder") || "");
  };
});
...
root.querySelectorAll("[data-action='add-comment']").forEach((addBtn) => {
  addBtn.onclick = async () => {
    ...
    const helpActive = String(ta?.dataset?.helpMode || "0") === "1" || isHelpTrigger(msgRaw);
    if (helpActive) {
      ta.value = "";
      // Ephemeral help: do NOT persist in comments/activities
    ...
    addComment(decisionTarget.type, decisionTarget.id, msgRaw);
    ta.value = "";
    refreshAll();

    // Mention-triggered LLM assist (@rapso) — public assistant reply
    if (/@rapso\b/i.test(msgRaw)) {
      await askRapsoAndAppendReply({ type: decisionTarget.type, id: decisionTarget.id, humanMessage: msgRaw });
    }
  };
});
...
// LLM webhooks (production) — IMPORTANT: ne pas inverser.
// - @rapso (réponse publique via mention) → rapsobot-poc-ask-llm
// - Assistant privé (overlay)            → rapsobot-poc-assistant
...
const ASSIST_AUTH_PHRASE = "autorisation directeur";
const ASSIST_CONFIG_URL = "./rapsobot_assistant.json";
...
function resetUI() {
  ...
  // PoC reset: clear private assistant overlay history
  try { localStorage.removeItem(STORAGE_KEY_ASSIST); } catch {}
...
/* ===== Private assistant overlay (full-screen slide-in from left) ===== */
function ensureAssistantOverlayDom() {
    ...
    <div class="assist-panel" role="dialog" aria-modal="true" aria-label="Assistant privé Rapso">
      ...
      <div class="assist-panel__body">
        ...
        <div class="assist-compose">
          <textarea id="assist-input" class="assist-input" rows="3" placeholder="Ex: Synthétise la situation SIT-ATT, ou prépare une validation en masse (D/S) sur le sujet PB-ATT-001… (les actions demanderont une autorisation directeur)"></textarea>
          <div class="assist-compose__actions">
            ...
            <button id="assist-send" class="assist-send" type="button" aria-label="Envoyer">
              <svg aria-hidden="true" viewBox="0 0 24 24" width="18" height="18" fill="none" style="display:block">
  ...
  // Decorative: reuse the top-left robot logo inside the assistant header when available
  try {
  ...
  // Close via X or click outside the panel
  overlay.querySelector(".assist-close").addEventListener("click", closeAssistantOverlay);
  overlay.addEventListener("click", (e) => {
    if (e.target === overlay) closeAssistantOverlay();
  });
  const panel = overlay.querySelector(".assist-panel");
  if (panel) {
    panel.addEventListener("click", (e) => e.stopPropagation());
  }

  function summarizePlannedToolCalls(toolCalls) {
  const lines = [];
  for (const tc of (toolCalls || [])) {
    const name = String(tc?.name || tc?.tool || "").trim() || "tool";
    const args = tc?.arguments || tc?.args || {};
    if (name === "set_avis_verdict") {
      lines.push(`- ${name}: ${String(args.avis_id||"").trim()} → ${String(args.verdict||"").trim().toUpperCase()}`);
    } else if (name === "bulk_set_avis_verdict_by_sujet") {
      lines.push(`- ${name}: ${String(args.sujet_id||"").trim()} (D/S) → ${String(args.verdict||"").trim().toUpperCase()}`);
    } else if (name === "set_sujet_status") {
      lines.push(`- ${name}: ${String(args.sujet_id||"").trim()} → ${String(args.status||"").trim().toLowerCase()}`);
    } else if (name === "bulk_set_sujet_status_by_situation") {
      lines.push(`- ${name}: ${String(args.situation_id||"").trim()} → ${String(args.status||"").trim().toLowerCase()}`);
    } else if (name === "bulk_set_sujet_status") {
      const n = Array.isArray(args.sujet_ids) ? args.sujet_ids.length : 0;
      lines.push(`- ${name}: ${n} sujets → ${String(args.status||"").trim().toLowerCase()}`);
    } else if (name === "set_situation_status") {
      lines.push(`- ${name}: ${String(args.situation_id||"").trim()} → ${String(args.status||"").trim().toLowerCase()}`);
    } else if (name === "bulk_set_situation_status") {
      const n = Array.isArray(args.situation_ids) ? args.situation_ids.length : 0;
      lines.push(`- ${name}: ${n} situations → ${String(args.status||"").trim().toLowerCase()}`);
    } else {
      lines.push(`- ${name}`);
    }
  }
  ...
  function executePendingAssistantActions() {
    const pending = getPendingAssistantActions();
    const toolCalls = Array.isArray(pending?.tool_calls) ? pending.tool_calls : [];
    if (!toolCalls.length) return;

    for (const tc of toolCalls) {
      applyAssistantToolCall(tc, { actor: "RAPSOBOT", agent: "specialist_ps" });
    }
    clearPendingAssistantActions();

    // Inform in private chat
    pushAssistChat({
      role: "assistant",
      actor: "RAPSOBOT",
      agent: "specialist_ps",
      message: [
        ...
        "",
        "Détail:",
        summarizePlannedToolCalls(toolCalls) || "- (aucune)"
      ].join("\n")
    });
  ...
  overlay.querySelector("#assist-send").addEventListener("click", () => {
    const ta = overlay.querySelector("#assist-input");
    const msgRaw = (ta.value || "").trim();
    if (!msgRaw) return;

    // Authorization path: phrase typed
    if (msgRaw.toLowerCase() === ASSIST_AUTH_PHRASE && hasPendingAssistantActions()) {
      ta.value = "";
      pushAssistChat({ role: "user", actor: "Human", agent: "human", message: msgRaw });
      renderAssistantOverlay();
      executePendingAssistantActions();
      return;
    }

    const helpOneShot = isHelpTrigger(msgRaw);
    const helpActive = !!(state.assistant && state.assistant.helpMode) || helpOneShot;

    const cleanMsg = helpActive ? (stripHelpTag(msgRaw) || "Explique-moi ce que je peux faire ici.") : msgRaw;

    ta.value = "";
    pushAssistChat({ role: "user", actor: "Human", agent: "human", message: cleanMsg, meta: helpActive ? { help: true } : {} });
    renderAssistantOverlay();

    if (helpActive) {
      const snap = buildUiSnapshot({ scope: "assistant_overlay" });
      const wrapped = [
        "MODE_HELP (assistant_overlay):",
        "Réponds très concret et court: Où suis-je / actions / exemples.",
        "",
        `UI snapshot: ${JSON.stringify(snap)}`,
        "",
        cleanMsg
      ].join("\n");
      askRapsoAssistant({ userMessage: wrapped, meta: { help: true } });
      return;
    }

    askRapsoAssistant({ userMessage: msgRaw });
  });


const helpBtn = overlay.querySelector("#assist-help-toggle");
if (helpBtn) {
  helpBtn.addEventListener("click", () => {
    state.assistant ||= {};
    state.assistant.helpMode = !state.assistant.helpMode;
    const on = !!state.assistant.helpMode;
    helpBtn.setAttribute("aria-pressed", on ? "true" : "false");
    helpBtn.classList.toggle("is-on", on);
    const ta = overlay.querySelector("#assist-input");
    if (ta) {
      ta.classList.toggle("helpMode", on);
      ta.setAttribute("placeholder", on
        ? "Mode Help activé — décrivez l’écran / l’action souhaitée. (Ctrl+Enter pour envoyer) — /help ou @help fonctionne aussi."
        : "Ex: Synthétise la situation SIT-ATT, ou prépare une validation en masse (D/S) sur le sujet PB-ATT-001… (les actions demanderont une autorisation directeur)");
    }
  });
}

  const authBtn = overlay.querySelector("#assist-authorize");
  if (authBtn) {
    authBtn.addEventListener("click", () => {
      if (!hasPendingAssistantActions()) return;
      // record explicit authorization in chat, then execute
      pushAssistChat({ role: "user", actor: "Human", agent: "human", message: ASSIST_AUTH_PHRASE });
      renderAssistantOverlay();
      executePendingAssistantActions();
    });
  }

  // Enter to send (Ctrl+Enter), keep normal Enter for new lines
  overlay.querySelector("#assist-input").addEventListener("keydown", (e) => {
...
function openAssistantOverlay() {
  ...
  // Best-effort: load assistant profile/config from repo
  loadAssistantConfig();
...
function closeAssistantOverlay() {
  ...
  const el = document.getElementById("assist-overlay");
...
function renderAssistantOverlay() {
  ...
  // Sync Help mode UI state (focus ring, placeholder, toggle)
  const helpOn = !!(state.assistant && state.assistant.helpMode);
  overlay.classList.toggle("assist-overlay--help", helpOn);
  try {
    const helpBtn = overlay.querySelector("#assist-help-toggle");
    if (helpBtn) {
      helpBtn.setAttribute("aria-pressed", helpOn ? "true" : "false");
      helpBtn.classList.toggle("is-on", helpOn);
    }
    const ta = overlay.querySelector("#assist-input");
    if (ta) {
      ta.classList.toggle("helpMode", helpOn);
      ta.setAttribute("placeholder", helpOn
        ? "Mode Help activé — décrivez l’écran / l’action souhaitée. (Ctrl+Enter pour envoyer) — /help ou @help fonctionne aussi."
        : "Ex: Synthétise la situation SIT-ATT, ou prépare une validation en masse (D/S) sur le sujet PB-ATT-001… (les actions demanderont une autorisation directeur)");
    }
  } catch {}

  const chat = getAssistChat();
  if (!chat.length) {
    thread.innerHTML = `<div class="assist-empty">
      <div class="assist-empty__title">Aucun échange pour l’instant</div>
      <div class="assist-empty__sub">Vous pouvez piloter le projet (ex: valider en masse) et demander des synthèses contextualisées.</div>
    </div>`;
    return;
  }
  ...
  for (const ev of chat) {
    ...
    const who = role === "user" ? "Vous" : (role === "assistant" ? "Rapso" : "Système");
    const ts = fmtDateTime(ev.ts);
    const isPending = !!ev?.meta?.pending;
    const rawMsg = String(ev.message || "");
    const cleanMsg = rawMsg.replace(/^_+|_+$/g, "");
    const bodyHtml = (isPending && role === "assistant")
      ? `<div>
          <div class="rapso-wait">
            <span class="rapso-spinner" aria-hidden="true"></span>
            <span class="rapso-shimmer">${escapeHtml(cleanMsg || "RAPSOBOT est en train de réfléchir…")}</span>
          </div>
          <div class="rapso-wait-sub">On revient vers vous dans quelques instants.</div>
         </div>`
      : mdToHtml(cleanMsg);
    ...
    html += `
      <div class="assist-msg assist-msg--${role} ${ev?.meta?.help ? "assist-msg--help" : ""}">
        <div class="assist-msg__meta">
          <span class="assist-msg__who">${escapeHtml(who)}</span>
          <span class="assist-msg__sep">•</span>
          <span class="assist-msg__ts">${escapeHtml(ts)}</span>
          ${isPending ? `<span class="assist-msg__pending">…</span>` : ``}
        </div>
        <div class="assist-msg__body">${bodyHtml}</div>
      </div>
    `;
  }
  ...
  try {
    ...
    if (authBtn) {
      const pending = getPendingAssistantActions();
      const has = pending && Array.isArray(pending.tool_calls) && pending.tool_calls.length;
      authBtn.classList.toggle("hidden", !has);
    }
  } catch {}
...
function wireEvents() {
  ...
  // Open private assistant overlay from top-left robot icon (GitHub-like brand logo)
// Also allow clicking on the brand text (name / sep / repo) to open the same overlay.
  document.addEventListener("click", (e) => {
    const t = e.target;
    const brandHit = t && t.closest ? t.closest(".gh-brand__logo, .gh-brand__name, .gh-brand__sep, .gh-brand__repo") : null;
    if (brandHit) {
      // do not interfere with sidebar toggle; assistant overlay is a separate full-screen panel
