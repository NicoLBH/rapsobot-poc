...
function fmtTs(ts) {
  try {
    ...
    // GitHub-like: YYYY-MM-DD HH:MM (local)
...
function getThreadForSelection() {
  ...
  if (a) {
    // viewing an avis: comments only on this avis; activities on parent sujet (+ situation for context)
  ...
  const filteredHuman = humanEvents.filter((e) => {
    ...
    if (t === "ACTIVITY") return allowedActivities.has(k);
...
function renderDetails(opts) {
  ...
  if (a) {
    ...
    setTitleHtml(`<span class="${badgeCls} gh-verdict-pill">${escapeHtml(v)}</span><span class="details-title-text"> ${titleText}</span><span class="details-title-id mono"> #${idText}</span>`);
  } else if (p) {
    const titleText = escapeHtml(p.topic ? p.topic : p.problem_id);
    const idText = escapeHtml(p.problem_id);
    setTitleHtml(`${statusBadgeHtml(getEffectiveProblemStatus(p.problem_id))}<span class="details-title-text"> ${titleText}</span><span class="details-title-id mono"> #${idText}</span>`);
  } else if (s) {
    const titleText = escapeHtml(s.title ? s.title : s.situation_id);
    const idText = escapeHtml(s.situation_id);
    setTitleHtml(`${statusBadgeHtml(getEffectiveSituationStatus(s.situation_id))}<span class="details-title-text"> ${titleText}</span><span class="details-title-id mono"> #${idText}</span>`);
  } else {
  ...
  const threadItemsHtml = thread.map((e, idx) => {
    ...
    if (t === "ACTIVITY") {
      ...
      if (kind === "issue_closed") {
        ...
        if (pbId) { verb = "closed"; target = `sujet #${pbId}`; }
        else { verb = "closed"; target = "this"; }
      }
      else if (kind === "issue_reopened") {
        icon = `<span class="tl-ico-wrap tl-ico-reopened" aria-hidden="true">${SVG_TL_REOPENED}</span>`;
        const pbId = e?.meta?.problem_id;
        if (pbId) { verb = "reopened"; target = `sujet #${pbId}`; }
        else { verb = "reopened"; target = "this"; }
      }
          ...
          <div class="thread-wrapper">
            <div class="tl-activity">
              ...
              <div class="tl-activity__text mono">
                ...
                <span class="mono-small"> ${escapeHtml(verb)} ${target ? escapeHtml(target) : ""} </span>
