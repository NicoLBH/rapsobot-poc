...
function applyQueryParamsToForm() {
  ...
  setVal("communeCp", qs.get("communeCp") || qs.get("commune_cp") || "");
  setVal("importance", qs.get("importance") || "je ne sais pas");
  setVal("soilClass", qs.get("soilClass") || qs.get("soil_class") || "je ne sais pas");
  setVal("liquefaction", qs.get("liquefaction") || "je ne sais pas");
  setVal("referential", qs.get("referential") || qs.get("referential_name") || "Eurocode 8");
  setVal("webhookUrl", qs.get("webhookUrl") || "");
}
...
function readInputs() {
  return {
    ...
    // webhookUrl input is optional (production uses a fixed URL)
...
function applyAssistantToolCall(tc, opts) {
  ...
  if (name === "set_avis_verdict") {
    ...
    // NOTE: setDecision() historise déjà une activity "avis_verdict_changed" sur la timeline du sujet parent.
    // On évite ici de dupliquer les notifications dans les timelines.
    if (args?.add_comment_to_sujet) {
      const d = state.data;
      const pid = d ? (buildParents(d).avisToProblem.get(avis_id) || null) : null;
      if (pid) {
  ...
  if (name === "set_sujet_status") {
    ...
    // NOTE: setDecision() historise déjà une activity issue_closed/issue_reopened (sur situation parente si sujet).
    // On évite ici de dupliquer les notifications dans les timelines.
  ...
  if (name === "set_situation_status") {
    ...
    // NOTE: setDecision() historise déjà issue_closed/issue_reopened sur la situation.
  ...
  if (name === "bulk_set_sujet_status_by_situation") {
    ...
    for (const pid of sujetIds) {
      applyAssistantToolCall({ name: "set_sujet_status", arguments: { sujet_id: pid, status: statusRaw, note, add_comment_to_sujet: false } }, opts);
    }
...
// Production webhooks (start + status)
    ...
    <div class="assist-panel" role="dialog" aria-modal="true" aria-label="Assistant privé Rapso">
      ...
      <div class="assist-panel__body">
        ...
        <div class="assist-compose">
          <textarea id="assist-input" class="assist-input" rows="3" placeholder="Ex: Synthétise la situation SIT-ATT, ou prépare une validation en masse (F/S/D/HM/PM/SO) sur le sujet PB-ATT-001… (les actions demanderont une autorisation directeur)"></textarea>
  ...
  function summarizePlannedToolCalls(toolCalls) {
    ...
    for (const tc of (toolCalls || [])) {
      ...
      if (name === "set_avis_verdict") {
        ...
        lines.push(`- ${name}: ${String(args.sujet_id||"").trim()} → ${String(args.verdict||"").trim().toUpperCase()}`);
      } else {
        lines.push(`- ${name}`);
      }
    }
  ...
  helpBtn.addEventListener("click", () => {
    ...
    if (ta) {
      ...
      ta.setAttribute("placeholder", on
        ...
        : "Ex: Synthétise la situation SIT-ATT, ou prépare une validation en masse (F/S/D/HM/PM/SO) sur le sujet PB-ATT-001… (les actions demanderont une autorisation directeur)");
    }
  });
}
