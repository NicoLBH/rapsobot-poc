...
function renderDetails(opts) {
  ...
  if (a) {
    ...
    setTitleHtml(`<span class="${badgeCls} gh-verdict-pill">${escapeHtml(v)}</span><span class="details-title-text"> ${titleText}</span><span class="details-title-id mono"> ${entityLinkHtml("avis", idText, "#"+idText)}</span>`);
  } else if (p) {
    const titleText = escapeHtml(p.topic ? p.topic : p.problem_id);
    const idText = escapeHtml(p.problem_id);
    setTitleHtml(`${statusBadgeHtml(getEffectiveProblemStatus(p.problem_id))}<span class="details-title-text"> ${titleText}</span><span class="details-title-id mono"> ${entityLinkHtml("problem", idText, "#"+idText)}</span>`);
  } else if (s) {
    const titleText = escapeHtml(s.title ? s.title : s.situation_id);
    const idText = escapeHtml(s.situation_id);
    setTitleHtml(`${statusBadgeHtml(getEffectiveSituationStatus(s.situation_id))}<span class="details-title-text"> ${titleText}</span><span class="details-title-id mono"> ${entityLinkHtml("situation", idText, "#"+idText)}</span>`);
  } else {
...
const sitPrioHtml = sitHasFilteredAvis
  ...
  : `<span class="${badgePriority(s.priority)}" style="visibility:hidden">${escapeHtml(s.priority || "P3")}</span>`;
    ...
    rows.push(`
      <div class="issue-row issue-row--sit click" data-action="select-sit" data-sit="${escapeHtml(s.situation_id)}">
...
const pbPrioHtml = pbHasFilteredAvis
  ...
  : `<span class="${badgePriority(pb.priority)}" style="visibility:hidden">${escapeHtml(pb.priority || "P3")}</span>`;
        ...
        rows.push(`
          <div class="issue-row issue-row--pb click" data-action="select-pb" data-pb="${escapeHtml(pb.problem_id)}">
