...
async function askRapsoAssistant({ userMessage }) {
  ...
  try {
    ...
    const toolCalls = Array.isArray(out?.planned_actions)
      ? out.planned_actions
      : (Array.isArray(out?.tool_calls) ? out.tool_calls : (Array.isArray(out?.tool_call) ? out.tool_call : null));
...
// Webhooks n8n
// - @rapso (expert parasismique, réponse publique via mention)  → /webhook-test/rapsobot-poc-ask-llm
// - Assistant privé (overlay "Assistant privé")               → /webhook-test/rapsobot-poc-assistant
// IMPORTANT: ne pas inverser ces deux endpoints (sinon collision entre workflows).
const ASK_LLM_URL_PROD = "https://nicolbh.app.n8n.cloud/webhook-test/rapsobot-poc-assistant";
const ASSIST_LLM_URL_PROD = "https://nicolbh.app.n8n.cloud/webhook-test/rapsobot-poc-ask-llm";
