...
function _helpFurtiveCommentHtml({ role = "assistant", bodyMd = "", pending = false } = {}) {
  ...
  return `
    <div class="gh-comment gh-comment--help gh-comment--${role}">
      <div class="gh-comment-avatar" aria-hidden="true">
        <svg viewBox="0 0 16 16" width="16" height="16" fill="currentColor" style="opacity:.9">
          <path d="M8 1a7 7 0 1 0 0 14A7 7 0 0 0 8 1Zm0 12.5A5.5 5.5 0 1 1 8 2.5a5.5 5.5 0 0 1 0 11Zm0-3.25a.75.75 0 1 0 0 1.5.75.75 0 0 0 0-1.5Zm.02-6.5c-1.1 0-1.9.66-1.9 1.63a.75.75 0 1 0 1.5 0c0-.14.13-.33.4-.33.27 0 .46.16.46.41 0 .22-.12.35-.4.62-.44.42-.85.84-.85 1.67v.18a.75.75 0 1 0 1.5 0v-.18c0-.23.08-.33.38-.62.47-.46.87-.86.87-1.67 0-1.12-.9-1.71-1.96-1.71Z"></path>
        </svg>
      </div>
...
function showEphemeralHelpThread(rootEl, { userMd, assistantPendingMd = "_RAPSOBOT réfléchit…_", ttlMs = 60_000 } = {}) {
...
async function askRapsoAssistant({ userMessage, meta = {} }) {
  ...
  pushAssistChat({ role: "assistant", actor: "RAPSOBOT", agent: "specialist_ps", message: "_RAPSOBOT analyse votre demande…_", meta: { ...(meta || {}), pending: true, request_id } });
...
function renderAssistantOverlay() {
  ...
  for (const ev of chat) {
    ...
    const bodyHtml = mdToHtml(ev.message || "");
