  ...
  const filteredHuman = humanEvents.filter((e) => {
    ...
    if (isViewingProblem) {
      ...
      if (String(e?.entity_type || "").toLowerCase() === "situation") {
        // IMPORTANT: do NOT let situation-level lifecycle activities (close/reopen of the situation)
        // "descend" into child sujets. Only keep situation activities that explicitly target this sujet.
        if (meta?.problem_id) return String(meta.problem_id) === String(p.problem_id);
        return false;
      }
      return true;
...
function situationHasFilteredAvis(s) {
  ...
  // Highlight the row corresponding to what is currently displayed in the details panel.
  // Only one row is marked as selected (the most specific: avis > sujet > situation).
  const selSid = state.selectedSituationId;
  const selPid = state.selectedProblemId;
  const selAid = state.selectedAvisId;
  const selectedKind = selAid ? "avis" : (selPid ? "problem" : (selSid ? "situation" : null));
...
const sitPrioHtml = sitHasFilteredAvis
  ...
  : `<span class="${badgePriority(s.priority)}" style="visibility:hidden">${escapeHtml(s.priority || "P3")}</span>`;
    ...
    rows.push(`
      <div class="issue-row issue-row--sit click${(selectedKind === "situation" && String(selSid) === String(s.situation_id)) ? " selected" : ""}" data-action="select-sit" data-sit="${escapeHtml(s.situation_id)}">
...
const pbPrioHtml = pbHasFilteredAvis
  ...
  : `<span class="${badgePriority(pb.priority)}" style="visibility:hidden">${escapeHtml(pb.priority || "P3")}</span>`;
        ...
        rows.push(`
          <div class="issue-row issue-row--pb click${(selectedKind === "problem" && String(selPid) === String(pb.problem_id)) ? " selected" : ""}" data-action="select-pb" data-pb="${escapeHtml(pb.problem_id)}">
        ...
        if (pbOpen) {
          ...
          for (const a of avisFiltered) {
            rows.push(`
              <div class="issue-row issue-row--avis click${(selectedKind === "avis" && String(selAid) === String(a.avis_id)) ? " selected" : ""}" data-action="select-avis" data-avis="${escapeHtml(a.avis_id)}">
