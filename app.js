...
function buildRapsoAssistantContext(userMessage) {
  ...
  if (!d) return {
    run_id: null,
    selection: { only_verdicts: ["F","D","S","HM","PM","SO"] },
    project: { situations: [] },
...
function applyAssistantToolCall(tc, opts) {
  ...
  if (name === "set_avis_verdict") {
    ...
    // Log activity to additional timelines beyond the built-in setDecision() activity.
    // setDecision("avis", ...) already writes an activity in the *parent sujet* timeline (or avis if no parent).
    const d = state.data;
    if (d) {
      const parents = buildParents(d);
      ...
      // Keep a trace on the avis timeline itself.
      logActivityToEntity("avis", avis_id, "avis_verdict_changed", note, { avis_id, to: verdict, problem_id: pid, situation_id: sid }, { actor, agent, ts });

      // And on the parent situation timeline (setDecision doesn't write there for avis changes).
  ...
  if (name === "set_sujet_status") {
    ...
    const decision = (statusRaw === "CLOSED") ? "CLOSED" : (statusRaw === "REOPENED" || statusRaw === "OPEN") ? "REOPENED" : "";
    ...
    // setDecision("problem", ...) already writes the close/reopen activity in the *parent situation* timeline.
    // Avoid duplicating it here.

    if (args?.add_comment_to_sujet) {
      addAgentComment("problem", sujet_id, note ? `Statut mis à jour — ${note}` : "Statut mis à jour.", agent, { tool: name, ts, from_assistant: true });
    }
  ...
  if (name === "set_situation_status") {
    ...
    // setDecision("situation", ...) already writes the close/reopen activity on the situation timeline.
    // Avoid duplicating it here.
  ...
  if (name === "bulk_set_sujet_status") {
    ...
    for (const pid of sujetIds) {
      applyAssistantToolCall({ name: "set_sujet_status", arguments: { sujet_id: pid, status: statusRaw, note, add_comment_to_sujet: !!args?.add_comment_to_sujets } }, opts);
    }
    return { ok: true, meta: { updated: sujetIds.length } };
  }
    ...
    <div class="assist-panel" role="dialog" aria-modal="true" aria-label="Assistant privé Rapso">
      ...
      <div class="assist-panel__body">
        ...
        <div class="assist-compose">
          <textarea id="assist-input" class="assist-input" rows="3" placeholder="Ex: Synthétise la situation SIT-ATT, ou prépare une validation en masse (F/D/S/HM/PM/SO) sur le sujet PB-ATT-001… (les actions demanderont une autorisation directeur)"></textarea>
  ...
  function summarizePlannedToolCalls(toolCalls) {
    ...
    for (const tc of (toolCalls || [])) {
      ...
      if (name === "set_avis_verdict") {
        ...
        lines.push(`- ${name}: ${String(args.sujet_id||"").trim()} (tous verdicts) → ${String(args.verdict||"").trim().toUpperCase()}`);
      } else {
        lines.push(`- ${name}`);
      }
    }
  ...
  helpBtn.addEventListener("click", () => {
    ...
    if (ta) {
      ...
      ta.setAttribute("placeholder", on
        ...
        : "Ex: Synthétise la situation SIT-ATT, ou prépare une validation en masse (F/D/S/HM/PM/SO) sur le sujet PB-ATT-001… (les actions demanderont une autorisation directeur)");
    }
  });
}
