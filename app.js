...
async function pollRunStatus({ statusUrl, runId }) {
  ...
  while (Date.now() - t0 < POLL_MAX_MS) {
    ...
    showError("Analyse en cours… (le workflow n8n tourne, l’UI attend le résultat)");
    setRunMeta(runId);
  ...
  // Timeout polling
  showError("Analyse toujours en cours ou résultat non récupéré. Réessaie (ou vérifie l’endpoint /rapsobot-poc-status).");
...
async function run() {
  ...
  const statusUrl = deriveStatusUrl(inp.webhookUrl);
  ...
  try {
    ...
    showError("Analyse en cours… (ack reçu, récupération du résultat)");
    ...
    if (isAbortError(e)) {
      showError("Analyse en cours… (timeout navigateur). Je tente de récupérer le résultat via le statut…");
      setSystemStatus("running", "En cours d’analyse", "timeout POST → polling");
    } else if (String(msg).toLowerCase().includes("failed to fetch")) {
      showError("Connexion instable : le POST a échoué côté navigateur. Je tente de récupérer le résultat via le statut…");
