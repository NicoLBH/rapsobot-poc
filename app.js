...
function applyAssistantToolCall(tc, opts) {
  ...
  if (name === "set_sujet_status") {
    ...
    const note = String(args?.note || "").trim();
    if (!sujet_id || !statusRaw) return { ok: false, error: "missing sujet_id or status" };

    const decision = (statusRaw === "CLOSED" || statusRaw === "CLOSE") ? "CLOSED"
      : (statusRaw === "OPEN" || statusRaw === "REOPEN" || statusRaw === "REOPENED") ? "REOPENED"
      : "";
    if (!decision) return { ok: false, error: "invalid status (expected OPEN|CLOSED|REOPENED)" };

    setDecision("problem", sujet_id, decision, note, { actor, agent, ts });

    logActivityToEntity("problem", sujet_id, "sujet_status_changed", note, { to: statusRaw }, { actor, agent, ts });

    // Also log to parent situation timeline
    const d = state.data;
    if (d) {
      const sid = buildParents(d).problemToSituation.get(sujet_id) || null;
      if (sid) logActivityToEntity("situation", sid, "sujet_status_changed", note, { problem_id: sujet_id, to: statusRaw }, { actor, agent, ts });
    }

    const addCommentToSituation = !!(args?.add_comment_to_situation ?? args?.add_comment_to_sujet);
    if (addCommentToSituation) {
      const d2 = state.data;
      const sid2 = d2 ? (buildParents(d2).problemToSituation.get(sujet_id) || null) : null;
      addAgentComment(sid2 ? "situation" : "problem", sid2 || sujet_id, note ? `Statut mis à jour — ${note}` : "Statut mis à jour.", agent, { tool: name, ts, from_assistant: true });
    }
  ...
  if (name === "set_situation_status") {
    ...
    const statusRaw = String(args?.status || "").trim().toUpperCase();
    const note = String(args?.note || "").trim();
    if (!situation_id || !statusRaw) return { ok: false, error: "missing situation_id or status" };
    ...
    logActivityToEntity("situation", situation_id, "situation_status_changed", note, { to: statusRaw }, { actor, agent, ts });

    if (args?.add_comment_to_situation) {
      addAgentComment("situation", situation_id, note ? `Statut mis à jour — ${note}` : "Statut mis à jour.", agent, { tool: name, ts, from_assistant: true });
    }
  ...
  if (name === "bulk_set_sujet_status_by_situation") {
    ...
    if (!sit) return { ok: false, error: "unknown situation_id" };

    const sujetIds = (sit.problem_ids || []).map(idFromAny).filter(Boolean);
    for (const pid of sujetIds) {
      applyAssistantToolCall({ name: "set_sujet_status", arguments: { sujet_id: pid, status: statusRaw, note, add_comment_to_situation: false } }, opts);
    }

    if (args?.add_comment_to_situation) {
      addAgentComment("situation", situation_id, note ? `Mise à jour des sujets (validation en masse) — ${note}` : "Mise à jour des sujets (validation en masse).", agent, { tool: name, ts, from_assistant: true });
    }

    return { ok: true, meta: { updated: sujetIds.length } };
  }

  if (name === "bulk_set_sujet_status") {
    const statusRaw = String(args?.status || "").trim().toUpperCase();
    const note = String(args?.note || "").trim();
    const sujetIds = Array.isArray(args?.sujet_ids) ? args.sujet_ids.map((x) => String(x || "").trim()).filter(Boolean) : [];
    if (!sujetIds.length || !statusRaw) return { ok: false, error: "missing sujet_ids or status" };

    for (const pid of sujetIds) {
      applyAssistantToolCall({ name: "set_sujet_status", arguments: { sujet_id: pid, status: statusRaw, note, add_comment_to_situation: !!(args?.add_comment_to_situation ?? args?.add_comment_to_situations) } }, opts);
    }
    return { ok: true, meta: { updated: sujetIds.length } };
  }
  ...
  if (name === "bulk_set_situation_status") {
    const statusRaw = String(args?.status || "").trim().toUpperCase();
    const note = String(args?.note || "").trim();
    const situationIds = Array.isArray(args?.situation_ids) ? args.situation_ids.map((x) => String(x || "").trim()).filter(Boolean) : [];
    if (!situationIds.length || !statusRaw) return { ok: false, error: "missing situation_ids or status" };

    for (const sid of situationIds) {
      applyAssistantToolCall({ name: "set_situation_status", arguments: { situation_id: sid, status: statusRaw, note, add_comment_to_situation: false } }, opts);
    }
    return { ok: true, meta: { updated: situationIds.length } };
  }
