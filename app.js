...
function renderDetails() {
  ...
  const buildSubIssuesForProblem = (pb, selectedAvisId) => {
    ...
    const COLS = `
      <div class="issues-table__head">
        ...
        <div class="cell cell-prio">Sev</div>
    ...
    const rows = avisFiltered.map((a2) => {
      ...
      return `
        <div class="issue-row issue-row--avis click${sel}" data-action="right-select-avis" data-avis="${escapeHtml(a2.avis_id)}">
          ...
          <div class="cell cell-verdict"><span class="${badgeVerdict(a2.verdict)}">${escapeHtml(String(a2.verdict || "").toUpperCase())}</span></div>
          <div class="cell cell-prio"><span class="${badgePriority(a2.severity)}">${escapeHtml(a2.severity || "")}</span></div>
          <div class="cell cell-agent mono-small">${escapeHtml(inferAgent(a2))}</div>
          <div class="cell cell-id mono">${escapeHtml(a2.avis_id)}</div>
        </div>
      `;
    }).join("");
  ...
  const buildSubIssuesForSituation = (sit) => {
    ...
    for (const pb of problems) {
      ...
      const chevHtml = hasAvis ? `<span class="chev click" data-action="right-toggle-pb" data-pb="${escapeHtml(pb.problem_id)}">${chev}</span>`
                               : `<span class="chev chev--spacer"></span>`;
      ...
      rows.push(`
        <div class="issue-row issue-row--pb click" data-action="right-select-pb" data-pb="${escapeHtml(pb.problem_id)}">
          ...
          <div class="cell cell-agent mono-small">${escapeHtml(inferAgent(pb))}</div>
