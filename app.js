...
const state = {
  ...
  // slide-in details panel (from right) for subissue clicks
  slideOpen: false,
  slideSelectedSituationId: null,
  slideSelectedProblemId: null,
  slideSelectedAvisId: null,
...
function progressPieSvg(fraction){
  // fraction in [0,1]
  const f = Math.max(0, Math.min(1, Number(fraction)||0));
  if (f <= 0.0001){
    return `<svg aria-hidden="true" focusable="false" class="octicon" viewBox="0 0 16 16" width="16" height="16" style="display:block"><circle cx="8" cy="8" r="7" fill="rgba(139,148,158,.22)" stroke="rgba(240,246,252,.18)" stroke-width="1"></circle></svg>`;
  }
  if (f >= 0.9999){
    // treat as closed
    return `<span class="subissues-progress-icon subissues-progress-icon--closed" aria-hidden="true">${SVG_ISSUE_CLOSED}</span>`;
  }
  const cx=8, cy=8, r=7;
  ...
  return `<svg aria-hidden="true" focusable="false" class="subissues-progress-icon" viewBox="0 0 16 16" width="16" height="16" style="display:block">
    <circle cx="8" cy="8" r="7" fill="rgba(139,148,158,.22)" stroke="rgba(240,246,252,.18)" stroke-width="1"></circle>
    <path d="${d}" fill="rgba(171,125,248,.55)"></path>
  </svg>`;
}

function problemsProgressIconHtml(closedCount, totalCount){
  const total = Math.max(0, Number(totalCount)||0);
  const closed = Math.max(0, Number(closedCount)||0);
  if (total > 0 && closed >= total){
    return `<span class="subissues-progress-icon subissues-progress-icon--closed" aria-hidden="true">${SVG_ISSUE_CLOSED}</span>`;
  }
  const frac = total ? (closed/total) : 0;
  return progressPieSvg(frac);
}
...
function renderDetails() {
  ...
  const buildSubIssuesForSituation = (sit) => {
    ...
    const metaHtml = `<div class="subissues-head-meta">
      <div class=\"subissues-counts subissues-counts--problems\">${problemsProgressIconHtml(closedPb, totalPb)}<span class=\"subissues-count-closed mono\">${closedPb}</span>/<span class=\"subissues-count-total mono\">${totalPb}</span></div>
      ${verdictCountsHtml}
    </div>`;
...
/* ===== Slide-in Details Panel (subissue drilldown without changing main selection) ===== */
function ensureSlidePanelDom(){
  if (document.getElementById("detailSlide")) return;
  const wrap = document.createElement("div");
  wrap.id = "detailSlide";
  wrap.className = "detail-slide";
  wrap.innerHTML = `
    <div class="detail-slide__panel" role="dialog" aria-modal="true" aria-label="Détail (drilldown)">
      <div class="detail-slide__head">
        <button class="icon-btn detail-slide__close" id="detailSlideClose" aria-label="Fermer">×</button>
        <div class="detail-slide__title" id="detailSlideTitle"></div>
      </div>
      <div class="detail-slide__body" id="detailSlideBody"></div>
    </div>
  `;
  document.body.appendChild(wrap);

  el("detailSlideClose").onclick = closeSlidePanel;

  // click outside panel closes
  wrap.addEventListener("click", (ev) => {
    if (ev.target === wrap) closeSlidePanel();
  });
}

function openSlideForProblem(problemId){
  if (!state.data) return;
  ensureSlidePanelDom();
  const parents = buildParents(state.data);
  const sid = parents.problemToSituation.get(problemId) || null;

  state.slideOpen = true;
  state.slideSelectedSituationId = sid;
  state.slideSelectedProblemId = problemId;
  state.slideSelectedAvisId = null;

  document.getElementById("detailSlide").classList.add("is-open");
  renderSlidePanel();
}

function openSlideForAvis(avisId){
  if (!state.data) return;
  ensureSlidePanelDom();
  const parents = buildParents(state.data);
  const pid = parents.avisToProblem.get(avisId) || null;
  const sid = parents.avisToSituation.get(avisId) || null;

  state.slideOpen = true;
  state.slideSelectedSituationId = sid;
  state.slideSelectedProblemId = pid;
  state.slideSelectedAvisId = avisId;
...
function closeSlidePanel(){
  ...
  state.slideSelectedSituationId = null;
  state.slideSelectedProblemId = null;
  state.slideSelectedAvisId = null;

  const wrap = document.getElementById("detailSlide");
  if (wrap) wrap.classList.remove("is-open");
}

function slideSelectProblem(pid){
  if (!pid || !state.data) return;
  const parents = buildParents(state.data);
  state.slideSelectedSituationId = parents.problemToSituation.get(pid) || null;
  state.slideSelectedProblemId = pid;
  state.slideSelectedAvisId = null;
  renderSlidePanel();
}
function slideSelectAvis(aid){
  if (!aid || !state.data) return;
  const parents = buildParents(state.data);
  state.slideSelectedSituationId = parents.avisToSituation.get(aid) || null;
  state.slideSelectedProblemId = parents.avisToProblem.get(aid) || null;
  state.slideSelectedAvisId = aid;
  renderSlidePanel();
}
...
function renderSlidePanel(){
  ...
  const titleEl = document.getElementById("detailSlideTitle");
  const bodyEl = document.getElementById("detailSlideBody");

  const d = state.data;
  const s = findSituation(state.slideSelectedSituationId);
  const p = findProblem(state.slideSelectedProblemId);
  const a = findAvis(state.slideSelectedAvisId);
  ...
  if (a) {
    ...
    const titleText = escapeHtml(a.topic ? a.topic : a.avis_id);
    const idText = escapeHtml(a.avis_id);
    titleEl.innerHTML = `<span class="${badgeCls} gh-verdict-pill">${escapeHtml(v)}</span><span class="details-title-text"> ${titleText}</span><span class="details-title-id mono"> #${idText}</span>`;
  } else if (p) {
    const titleText = escapeHtml(p.topic ? p.topic : p.problem_id);
    const idText = escapeHtml(p.problem_id);
    titleEl.innerHTML = `${statusBadgeHtml(p.status)}<span class="details-title-text"> ${titleText}</span><span class="details-title-id mono"> #${idText}</span>`;
  } else if (s) {
    const titleText = escapeHtml(s.title ? s.title : s.situation_id);
    const idText = escapeHtml(s.situation_id);
    titleEl.innerHTML = `${statusBadgeHtml(s.status)}<span class="details-title-text"> ${titleText}</span><span class="details-title-id mono"> #${idText}</span>`;
  } else {
    titleEl.textContent = "Détail";
  }

  const metaItem = (label, valueHtml) => {
    const v = (valueHtml === undefined || valueHtml === null || String(valueHtml).trim() === "") ? "—" : String(valueHtml);
    return `<div class="meta-item"><div class="meta-k">${escapeHtml(label)}</div><div class="meta-v">${v}</div></div>`;
  };
  const wrapDetailsGrid = (mainHtml, metaHtml) => {
    const metaBlock = metaHtml && String(metaHtml).trim()
      ? `<aside class="details-meta-col"><div class="meta-title">Metadata</div>${metaHtml}</aside>`
  ...
  const subIssuesPanel = ({ title, metaHtml, bodyHtml }) => `
    <div class="details-subissues">
      <div class="subissues-head">
        ...
        <div class="subissues-head-right">${metaHtml || ""}</div>
      </div>
      <div class="subissues-body">${bodyHtml || ""}</div>
    </div>
  `;

  const buildSubIssuesForProblemSlide = (pb, selectedAvisId) => {
    const avById = indexBy(d.avis || [], "avis_id");
    const avisAll = (pb?.avis_ids || []).map((aid) => avById.get(idFromAny(aid))).filter(Boolean);
    const avisFiltered = applyAvisFilters(avisAll);
    ...
    const rows = avisFiltered.map((a2) => {
      const sel = (selectedAvisId && a2.avis_id === selectedAvisId) ? " subissue-row--selected" : "";
      return `
        <div class="issue-row issue-row--avis click${sel}" data-action="slide-select-avis" data-avis="${escapeHtml(a2.avis_id)}">
          <div class="cell cell-theme cell-theme--full lvl0">
            <span class="chev chev--spacer"></span>
            <span class="${verdictDotClass(a2.verdict)}" aria-hidden="true"></span>
            <span class="theme-text theme-text--avis">${escapeHtml(a2.topic || "")}</span>
    ...
    const metaHtml = `<div class="subissues-head-meta"><div class="subissues-counts subissues-counts--total"><span class="mono">${avisFiltered.length}</span></div></div>`;
    return subIssuesPanel({ title: "Avis rattachés", metaHtml, bodyHtml: body });
  };
  ...
  if (a) {
    ...
    metaHtml = [
      ps ? metaItem("Situation", escapeHtml(ps.title || "(sans titre)")) : "",
      ps ? metaItem("situation_id", `<span class="mono">${escapeHtml(ps.situation_id)}</span>`) : "",
      pp ? metaItem("Sujet", escapeHtml(pp.topic || "Non classé")) : "",
      pp ? metaItem("sujet_id", `<span class="mono">${escapeHtml(pp.problem_id)}</span>`) : "",
      metaItem("Severity", `<span class="${badgePriority(a.severity)}">${escapeHtml(a.severity)}</span>`),
      metaItem("Verdict", `<span class="${badgeVerdict(a.verdict)}">${escapeHtml(a.verdict)}</span>`),
      metaItem("Agent", `<span class="mono">${escapeHtml(inferAgent(a))}</span>`),
      metaItem("avis_id", `<span class="mono">${escapeHtml(a.avis_id || "")}</span>`),
    ...
    mainHtml = `
      ...
      <div style="white-space: pre-wrap; font-size: 13px;">${escapeHtml(a.message || "")}</div>
    `;
    if (pp) subHtml = buildSubIssuesForProblemSlide(pp, a.avis_id);
  } else if (p) {
    metaHtml = [
      s ? metaItem("Situation", escapeHtml(s.title || "(sans titre)")) : "",
      s ? metaItem("situation_id", `<span class="mono">${escapeHtml(s.situation_id)}</span>`) : "",
      metaItem("Priority", `<span class="${badgePriority(p.priority)}">${escapeHtml(p.priority || "")}</span>`),
      metaItem("Topic", escapeHtml(p.topic || "Non classé")),
      metaItem("Agent", `<span class="mono">${escapeHtml(inferAgent(p))}</span>`),
      metaItem("sujet_id", `<span class="mono">${escapeHtml(p.problem_id)}</span>`),
    ...
    mainHtml = `
      ${p.summary ? `<div class="mono" style="color: var(--muted); font-size:12px; margin-bottom:6px;">Synthèse</div><div style="white-space: pre-wrap; font-size: 13px;">${escapeHtml(p.summary)}</div>` : ``}
    `;
    subHtml = buildSubIssuesForProblemSlide(p, null);
  } else {
    bodyEl.innerHTML = `<div class="emptyState">Sélectionne un sujet ou un avis.</div>`;
    return;
  }

  const descAgent = a ? inferAgent(a) : inferAgent(p);
  const descCard = ghDescriptionCard(descAgent, mainHtml);
  const body = wrapDetailsGrid(`<div class="gh-timeline">${descCard}${subHtml}</div>`, metaHtml);

  bodyEl.innerHTML = body;

  bodyEl.querySelectorAll("[data-action='slide-select-avis']").forEach((node) => {
    node.onclick = () => {
      const aid = node.getAttribute("data-avis");
      if (!aid) return;
      slideSelectAvis(aid);
...
function wireEvents() {
  ...
  window.addEventListener("keydown", (ev) => {
    ...
    if (slide && slide.classList.contains("is-open")) { closeSlidePanel(); return; }
    closeDetailsModal();
  }
  });
