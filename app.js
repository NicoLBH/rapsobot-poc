...
function initRightSplitter() {
  ...
  // icon: two vertical bars + two arrows
  splitter.innerHTML = `
    <svg class="gh-splitter__icon" width="14" height="18" viewBox="0 0 14 18" fill="none" aria-hidden="true" focusable="false">
      <!-- bars -->
      <rect x="6" y="3" width="1.5" height="12" rx="0.75" fill="currentColor"></rect>
      <rect x="8.5" y="3" width="1.5" height="12" rx="0.75" fill="currentColor"></rect>
      <!-- left arrow -->
      <path d="M5 9 L2.5 6.5 M5 9 L2.5 11.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
      <!-- right arrow -->
      <path d="M9 9 L11.5 6.5 M9 9 L11.5 11.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
    </svg>
  `;
  ...
  const MAX_W = 750;
  const MIN_W = (() => {
    // derive from current computed width (fallback 420)
    const w = Math.round(details.getBoundingClientRect().width || 420);
    return Math.max(280, w); // never go below current default too much
  })();
