...
function setDecision(type, id, decision, note) {
  ...
  // Activity: issue closed / reopened (situations + sujets)
  ...
  // Activity: avis verdict change (validated_*), with from/to if possible
  if (type === "avis") {
...
function renderDetails(opts) {
  ...
  if (a) {
    ...
    setTitleHtml(`${statusBadgeHtml(p.status)}<span class="details-title-text"> ${titleText}</span><span class="details-title-id mono"> #${idText}</span>`);
  } else if (s) {
    const titleText = escapeHtml(s.title ? s.title : s.situation_id);
    const idText = escapeHtml(s.situation_id);
    setTitleHtml(`${statusBadgeHtml(s.status)}<span class="details-title-text"> ${titleText}</span><span class="details-title-id mono"> #${idText}</span>`);
  } else {
  ...
  const threadItemsHtml = thread.map((e, idx) => {
    ...
    if (t === "ACTIVITY") {
      ...
      if (kind === "issue_closed") { icon = `<span class="tl-ico-wrap tl-ico-closed" aria-hidden="true">${SVG_TL_CLOSED}</span>`; verb = "closed this"; }
      else if (kind === "issue_reopened") { icon = `<span class="tl-ico-wrap tl-ico-reopened" aria-hidden="true">${SVG_TL_REOPENED}</span>`; verb = "reopened this"; }
          ...
          <div class="thread-wrapper">
            <div class="tl-activity">
              ...
              <div class="tl-activity__text mono">
                ...
                <span class="mono-small"> ${escapeHtml(verb)} </span>
