...
if (t === "ACTIVITY") {
      ...
      // Ensure author display name is always defined for activity items
...
async function run() {
  ...
  try {
    ...
    const ok = await pollRunStatus({ statusUrl, runId });
    state.isRunning = false;
  } catch (e) {
...
function resetUI(opts) {
  opts = opts || {};
  // Prevent accidental reset (e.g., clicking the robot head) while an analysis is running.
  // Use resetUI({ force: true }) for explicit resets.
  if (state.isRunning && !opts.force) return;
...
function wireEvents() {
  ...
  el("resetBtnTop").onclick = () => resetUI({ force: true });
