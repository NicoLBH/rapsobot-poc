...
function _extractJsonStringField(rawText, fieldName) {
  ...
  const key = `"${String(fieldName || "").replace(/"/g, \'\"\' )}"`;
  const i = s.indexOf(key);
  if (i < 0) return null;
  // Match a JSON string literal: "value with escapes"
  const reStr = new RegExp(`"${fieldName}"\\s*:\\s*("([^"\\\\]|\\\\.)*")`);
  const m = s.match(reStr);
  if (!m) return null;
...
function _pickReplyMarkdown(out, rawText) {
  ...
  // Try parsing JSON from raw text (direct or fenced).
  const obj = rawText ? _normalizeLlmRawToObject(rawText) : null;
  if (obj) return _pickReplyMarkdown(obj, null);

  // Fallback: extract "reply_markdown" as a JSON string literal from the raw text.
  // (Useful when the response isn't valid JSON overall, but contains a reply_markdown field.)
  const extracted = rawText ? _extractJsonStringField(rawText, "reply_markdown") : null;
...
async function askRapsoAssistant({ userMessage }) {
  const ctx = buildRapsoAssistantContext(userMessage);
  const request_id = `assist_${Date.now()}`;

  // placeholder in assistant chat
  pushAssistChat({ role: "assistant", actor: "RAPSOBOT", agent: "specialist_ps", message: "_RAPSOBOT analyse votre demande…_", meta: { pending: true, request_id } });
...
overlay.addEventListener("click", (ev) => {
  ...
  overlay.querySelector("#assist-send").addEventListener("click", () => {
    ...
    if (msg.toLowerCase() === ASSIST_AUTH_PHRASE && hasPendingAssistantActions()) {
      ta.value = "";
      pushAssistChat({ role: "user", actor: "Human", agent: "human", message: msg });
      renderAssistantOverlay();
    ...
    pushAssistChat({ role: "user", actor: "Human", agent: "human", message: msg });
    renderAssistantOverlay();
    askRapsoAssistant({ userMessage: msg });
  });
...
function renderAssistantOverlay() {
  ...
  for (const ev of chat) {
    ...
    html += `
      <div class="assist-msg assist-msg--${role}">
