...
function buildRapsoContextBundle(type, id, humanMessage) {
  ...
  // siblings (light) for avis/prob scope
  ...
  // recent discussion messages in this exact scope
  const thread_recent = (() => {
    const store = loadHumanStore();
...
async function askRapsoAndAppendReply({ type, id, humanMessage }) {
  ...
  addAgentComment(type, id, "_specialist_ps is thinking…_", "specialist_ps", { pending: true, request_id });
  refreshAll();
...
function renderDetails(opts) {
  ...
  const buildSubIssuesForProblem = (pb, state.selectedAvisId) => {
    const avById = indexBy(d.avis || [], "avis_id");
    ...
    const rows = avisFiltered.map((a2) => {
      const sel = (state.selectedAvisId && a2.avis_id === state.selectedAvisId) ? " subissue-row--selected" : "";
      return `
...
if (isComment) {
  ...
  const avatar = isHuman
    ...
    : `<div class="gh-avatar" aria-hidden="true"><span class="gh-avatar-initial">${escapeHtml((agent[0] || "S").toUpperCase())}</span></div>`;
