...
function badgeVerdict(v) {
  ...
  // New scale: F / D / S / HM / PM / SO
  if (s === "F") return "badge badge--F";
  if (s === "D") return "badge badge--D";
  if (s === "S") return "badge badge--S";
  if (s === "HM") return "badge badge--HM";
  if (s === "PM") return "badge badge--PM";
  if (s === "SO") return "badge badge--SO";
...
function setTopAlert({ kind, message, status } = {}) {
  const bar = el("topAlert");
  if (!bar) return;
  ...
  if (!msg && !st) {
    ...
    bar.classList.remove("gh-infobar--info", "gh-infobar--error");
    bar.textContent = "";
  ...
  bar.classList.toggle("gh-infobar--error", kind === "error");
  bar.classList.toggle("gh-infobar--info", kind !== "error");

  bar.textContent = st ? `${msg} (status: ${st})` : msg;
}
    ...
    html.push(`
      <div class="line click" data-action="toggle-sit" data-sit="${escapeHtml(s.situation_id)}">
        ...
        <div class="line-meta">
          ...
          <span class="mono id-col">${escapeHtml(s.situation_id)}</span>
        </div>
      </div>
    `);
        ...
        html.push(`
          <div class="line click" style="padding-left:22px;" data-action="toggle-pb" data-pb="${escapeHtml(pb.problem_id)}">
            ...
            <div class="line-meta">
              ...
              <span class="mono id-col">${escapeHtml(pb.problem_id)}</span>
            </div>
          </div>
        `);
                ...
                <tbody>
                  ${slice.map((a) => `
                    <tr class="click" data-action="select-avis" data-avis="${escapeHtml(a.avis_id)}">
                      ...
                      <td style="width:70px;"><span class="prio-pill">${escapeHtml(a.severity || "")}</span></td>
                      <td style="width:140px;" class="mono-small">${escapeHtml(inferAgent(a))}</td>
                      <td style="width:140px;" class="mono id-col">${escapeHtml(a.avis_id)}</td>
                    </tr>
                  `).join("")}
                </tbody>
              </table>
...
async function pollRunStatus({ statusUrl, runId }) {
  ...
  while (Date.now() - t0 < POLL_MAX_MS) {
    tries++;
    setSystemStatus("running", "En cours d’analyse");
    ...
    try {
      ...
      setTopAlert({ kind: "info", message: "Analyse en cours… (récupération du statut en cours)", status: "IN_PROGRESS" });
    ...
    setTopAlert({ kind: "info", message: "Analyse en cours… (le workflow n8n tourne, l’UI attend le résultat)", status: meta });
    setSystemStatus("running", "En cours d’analyse");
  ...
  // Timeout polling
  setTopAlert({ kind: "error", message: "Analyse toujours en cours ou résultat non récupéré. Réessaie (ou vérifie l’endpoint /rapsobot-poc-status).", status: "TIMEOUT" });
  setSystemStatus("error", "Timeout");
  return false;
}
...
async function run() {
  ...
  if (!inp.webhookUrl) {
    setTopAlert({ kind: "error", message: "Webhook URL manquant. Renseigne-le dans le champ (ou via ?webhookUrl=...).", status: "ERROR" });
    setSystemStatus("error", "Erreur");
    return;
  }
  if (!inp.pdfFile) {
    setTopAlert({ kind: "error", message: "PDF manquant. Sélectionne un fichier PDF depuis l’ordinateur.", status: "ERROR" });
    setSystemStatus("error", "Erreur");
    return;
  }
  ...
  slowTimer = setTimeout(() => {
    setTopAlert({ kind: "info", message: "Analyse en cours… (cela peut prendre 1–3 minutes selon le PDF).", status: "IN_PROGRESS" });
    setSystemStatus("running", "En cours d’analyse");
  }, SLOW_NOTICE_MS);
  ...
  try {
    ...
    setTopAlert({ kind: "info", message: "Analyse en cours… (ack reçu, récupération du résultat)", status: "ACK" });
    setSystemStatus("running", "En cours d’analyse");
    ...
    if (isAbortError(e)) {
      setTopAlert({ kind: "info", message: "Analyse en cours… (timeout navigateur). Je tente de récupérer le résultat via le statut…", status: "IN_PROGRESS" });
      setSystemStatus("running", "En cours d’analyse");
    } else if (String(msg).toLowerCase().includes("failed to fetch")) {
      setTopAlert({ kind: "info", message: "Connexion instable : le POST a échoué côté navigateur. Je tente de récupérer le résultat via le statut…", status: "IN_PROGRESS" });
      setSystemStatus("running", "En cours d’analyse");
    } else {
      setTopAlert({ kind: "info", message: `POST webhook en erreur: ${msg}. Je tente quand même la récupération via le statut…`, status: "IN_PROGRESS" });
      setSystemStatus("running", "En cours d’analyse");
    }
...
function wireEvents() {
  ...
  if (el("resetBtn")) el("resetBtn").onclick = resetUI;
