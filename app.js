...
function renderDetails(opts) {
  ...
  const threadItemsHtml = thread.map((e, idx) => {
    ...
    if (t === "ACTIVITY") {
      ...
      if (kind === "issue_closed") { icon = `<span class="tl-ico-wrap" aria-hidden="true">${SVG_TL_CLOSED}</span>`; verb = "closed this"; }
      else if (kind === "issue_reopened") { icon = `<span class="tl-ico-wrap" aria-hidden="true">${SVG_TL_REOPENED}</span>`; verb = "reopened this"; }
