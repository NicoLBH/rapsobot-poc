...
function addAgentComment(type, id, message, agentName = "specialist_ps") {
  const store = loadHumanStore();
  const bucket = ensureRunBucket(store);
  ...
  // ensure there's at least a decision stub so this entity shows activity in store
  bucket.decisions[entityKey(type, id)] ||= { decision: "COMMENTED", note: "", ts: nowIso() };
  saveHumanStore(store);
}
...
function buildRapsoContextBundle(type, id, humanMessage) {
  ...
  // siblings (light)
  const siblings_summary = (() => {
    if (!problem) return [];
    const avById = indexBy(d.avis || [], "avis_id");
    return (problem.avis_ids || [])
      .map((aid) => avById.get(idFromAny(aid)))
      .filter(Boolean)
      .map((a) => ({
        avis_id: a.avis_id,
        topic: a.topic || "",
        verdict: String(getEffectiveAvisVerdict(a.avis_id) || a.verdict || "").toUpperCase(),
        severity: a.severity || "",
      }));
  })();

  // recent thread messages in this scope (public discussion)
  const thread_recent = (() => {
    const store = loadHumanStore();
...
async function askRapsoAndAppendReply({ type, id, humanMessage }) {
  ...
  try {
    ...
    addAgentComment(type, id, reply, "specialist_ps");
    refreshAll();
  } catch (e) {
    console.error("[RAPSOBOT] @rapso ask failed:", e);
    showError(`@rapso: échec de l'appel LLM (${e?.message || String(e)})`);
  }
}
...
const sitPrioHtml = sitHasFilteredAvis
  ...
  : `<span class="${badgePriority(s.priority)}" style="visibility:hidden">${escapeHtml(s.priority || "P3")}</span>`;
    ...
    rows.push(`
      <div class="issue-row issue-row--sit click${(selectedSituationId && s.situation_id === selectedSituationId) ? " selected" : ""}" data-action="select-sit" data-sit="${escapeHtml(s.situation_id)}">
...
const pbPrioHtml = pbHasFilteredAvis
  ...
  : `<span class="${badgePriority(pb.priority)}" style="visibility:hidden">${escapeHtml(pb.priority || "P3")}</span>`;
        ...
        rows.push(`
          <div class="issue-row issue-row--pb click${(selectedProblemId && pb.problem_id === selectedProblemId) ? " selected" : ""}" data-action="select-pb" data-pb="${escapeHtml(pb.problem_id)}">
        ...
        if (pbOpen) {
          ...
          for (const a of avisFiltered) {
            rows.push(`
              <div class="issue-row issue-row--avis click${(selectedAvisId && a.avis_id === selectedAvisId) ? " selected" : ""}" data-action="select-avis" data-avis="${escapeHtml(a.avis_id)}">
