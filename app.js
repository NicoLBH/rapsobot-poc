...
function renderDetails() {
  ...
  const buildSubIssuesForProblem = (pb, selectedAvisId) => {
    ...
    const rows = avisFiltered.map((a2) => {
      ...
      const verdict = String(a2.verdict || "").toUpperCase();
      const sev = String(a2.severity || "").toUpperCase();
      return `
        <div class="issue-row issue-row--avis click${sel}" data-action="right-select-avis" data-avis="${escapeHtml(a2.avis_id)}">
          <div class="cell cell-theme lvl0">
            <div class="theme-left" style="display:flex; align-items:center; gap:10px; min-width:0;">
              <span class="chev chev--spacer"></span>
              <span class="theme-text theme-text--avis">${escapeHtml(a2.topic || "")}</span>
            </div>
            <div class="theme-right">
              ${verdict ? `<span class="${badgeVerdict(verdict)}">${escapeHtml(verdict)}</span>` : ``}
              ${sev ? `<span class="${badgePriority(sev)}">${escapeHtml(sev)}</span>` : ``}
              <span class="subissues-meta mono">${escapeHtml(a2.avis_id)}</span>
            </div>
          </div>
        </div>
      `;
    ...
    const body = `
      <div class="issues-table subissues-table subissues-table--compact">
  ...
  const buildSubIssuesForSituation = (sit) => {
    ...
    for (const pb of problems) {
      ...
      const open = hasAvis && state.rightExpandedProblems.has(pb.problem_id);
      const chev = hasAvis ? (open ? "▾" : "▸") : "";
      const chevHtml = hasAvis
        ? `<span class="chev click" data-action="right-toggle-pb" data-pb="${escapeHtml(pb.problem_id)}">${chev}</span>`
        : `<span class="chev chev--spacer"></span>`;

      rows.push(`
        <div class="issue-row issue-row--pb click" data-action="right-select-pb" data-pb="${escapeHtml(pb.problem_id)}">
          <div class="cell cell-theme lvl0">
            <div class="theme-left" style="display:flex; align-items:center; gap:10px; min-width:0;">
              ${chevHtml}
            ...
            <div class="theme-right">
              <span class="subissues-meta mono">avis=${escapeHtml((pb.avis_ids || []).length)}</span>
              <span class="subissues-meta mono">${escapeHtml(pb.problem_id)}</span>
            </div>
          </div>
        </div>
      `);
      ...
      if (open) {
        ...
        for (const a2 of avisFiltered) {
          ...
          const sev = String(a2.severity || "").toUpperCase();
          rows.push(`
            <div class="issue-row issue-row--avis click" data-action="right-select-avis" data-avis="${escapeHtml(a2.avis_id)}">
  ...
  const mainWithThreadAndComment = `${descCard}${subIssuesHtml || ""}${threadHtml}{{COMMENT_BOX}}`;
