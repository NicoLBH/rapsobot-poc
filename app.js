...
function buildRapsoAssistantContext(userMessage) {
  ...
  return {
    ...
    response_constraints: {
      max_lines: 6,
      tone: "courtois, détendu, vouvoiement",
      format: "reply_markdown court (5–6 lignes)",
    },
    tool_call_contract: {
      mode: "A",
      expected: {
      ...
      allowed_tools: [
        ...
        },
        {
          ...
          args: { sujet_ids: "string[]", status: "open|closed", note: "string", add_comment_to_situation: "boolean" }
        },
        {
        ...
        },
        {
          ...
          args: { situation_ids: "string[]", status: "open|closed", note: "string" }
        }
      ]},
    user_message: String(userMessage || "").trim(),
  };
...
function applyAssistantToolCall(tc, opts) {
  ...
  // ===== Sujets (Problems) status =====
  if (name === "set_sujet_status") {
    const sujet_id = String(args?.sujet_id || "").trim();
    const status = String(args?.status || "").toLowerCase();
    const note = String(args?.note || "").trim();
    if (!sujet_id || (status !== "open" && status !== "closed")) return { ok: false, error: "missing sujet_id or invalid status" };
    ...
    if (args?.add_comment_to_situation) {
      ...
      if (d) {
        const sid = buildParents(d).problemToSituation.get(sujet_id) || null;
        const targetType = sid ? "situation" : "problem";
        const targetId = sid || sujet_id;
        const msg = note ? `${status === "closed" ? "Clôture" : "Réouverture"} (par Human) — ${note}` : `${status === "closed" ? "Clôture" : "Réouverture"} (par Human).`;
        addAgentComment(targetType, targetId, msg, agent, { tool: name, ts, from_assistant: true });
      }
    }
  ...
  if (name === "bulk_set_sujet_status_by_situation") {
    ...
    const status = String(args?.status || "").toLowerCase();
    const note = String(args?.note || "").trim();
    if (!situation_id || (status !== "open" && status !== "closed")) return { ok: false, error: "missing situation_id or invalid status" };
    ...
    for (const pid of sujetIds) {
      const baseS = String(getEffectiveProblemStatus(pid) || "open").toLowerCase();
      if (baseS === status) continue;
      applyAssistantToolCall({ name: "set_sujet_status", arguments: { sujet_id: pid, status, note, add_comment_to_situation: false } }, opts);
      updated++;
    }

    if (args?.add_comment_to_situation) {
      const msg = note ? `${status === "closed" ? "Clôture" : "Réouverture"} en masse (par Human) — ${note}` : `${status === "closed" ? "Clôture" : "Réouverture"} en masse (par Human).`;
      addAgentComment("situation", situation_id, msg, agent, { tool: name, ts, from_assistant: true });
    }
  ...
  if (name === "bulk_set_sujet_status") {
    ...
    const status = String(args?.status || "").toLowerCase();
    const note = String(args?.note || "").trim();
    if (!sujet_ids.length || (status !== "open" && status !== "closed")) return { ok: false, error: "missing sujet_ids or invalid status" };

    let updated = 0;
    for (const pidRaw of sujet_ids) {
    ...
    // add_comment_to_situation: best-effort — comment on first parent situation if all share same, else skip
    if (args?.add_comment_to_situation) {
      const d = state.data;
      if (d) {
        const parents = buildParents(d);
        ...
        if (sids.size === 1) {
          const onlySid = Array.from(sids)[0];
          const msg = note ? `${status === "closed" ? "Clôture" : "Réouverture"} en masse (par Human) — ${note}` : `${status === "closed" ? "Clôture" : "Réouverture"} en masse (par Human).`;
          addAgentComment("situation", onlySid, msg, agent, { tool: name, ts, from_assistant: true });
        }
      }
    }
  ...
  if (name === "bulk_set_situation_status") {
    ...
    const status = String(args?.status || "").toLowerCase();
    const note = String(args?.note || "").trim();
    if (!situation_ids.length || (status !== "open" && status !== "closed")) return { ok: false, error: "missing situation_ids or invalid status" };

    let updated = 0;
    for (const sidRaw of situation_ids) {
      const sid = String(sidRaw || "").trim();
      if (!sid) continue;
      const baseS = String(getEffectiveSituationStatus(sid) || "open").toLowerCase();
      if (baseS === status) continue;
...
async function askRapsoAssistant({ userMessage }) {
  ...
  try {
    ...
    const reply = String(out?.reply_markdown || out?.reply || "").trim() || "_(no reply)_";
    ...
    const toolCalls = Array.isArray(out?.tool_calls) ? out.tool_calls : (Array.isArray(out?.tool_call) ? out.tool_call : null);
    ...
    <div class="assist-panel" role="dialog" aria-modal="true" aria-label="Assistant privé Rapso">
      ...
      <div class="assist-panel__body">
        ...
        <div class="assist-compose">
          <textarea id="assist-input" class="assist-input" rows="3" placeholder="Ex: Mettre tous les avis D/S du sujet PROB-123 en Favorable, et noter que c'est validé par Human…"></textarea>
          <div class="assist-compose__actions">
            <div class="assist-hint">Réponse courte (5–6 lignes). Utilise le même webhook que @rapso.</div>
            <button id="assist-send" class="btn btn-primary" type="button">Envoyer</button>
          </div>
        </div>
      </div>
    </div>
