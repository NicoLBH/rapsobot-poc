...
const state = {
  ...
  verdictFilter: "ALL",
  search: "",
  displayDepth: "avis", // situations | sujets | avis
...
function injectDepthControl() {
  // Insert "Depth" dropdown to the right of Search
  ...
  // Basic style (keeps alignment with existing controls)
  if (!document.getElementById("depth-control-style")) {
    const style = document.createElement("style");
    style.id = "depth-control-style";
    style.textContent = `
      .depthSelect {
        margin-left: 8px;
        height: 30px;
  ...
  sel.value = state.displayDepth || "avis";
...
function renderMiddle() {
  ...
  for (const s of d.situations) {
    ...
    const sitOpen = (state.displayDepth !== "situations") && hasProblems && state.expandedSituations.has(s.situation_id);
...
try {
  ...
  continue;
    ...
    if ((status === "READY_FOR_REVIEW" || status === "DONE" || status === "READY") && payload) {
      ...
      setDisplayDepth(state.displayDepth || "avis");
      return true;
    }
...
function resetUI() {
  ...
  state.displayDepth = "avis";

  el("verdictFilter").value = "ALL";
  el("searchBox").value = "";
  if (el("depthSelect")) el("depthSelect").value = state.displayDepth;
